...
最近更新：2026-08-10（完成 M5，并完成 M6 评论系统基础闭环）
...
| M6-2 | 前端路由深化 | 高 | 进行中 | - | 前端 | 当前主工作区已使用 react-router-dom，下一步迁移库/视图/筛选状态到 URL 参数并收敛 Layout 路由边界 |
| M6-3 | 后端统一错误处理 | 高 | 进行中 | - | 后端 | 全局 AppException/HTTP/校验异常处理器已存在，下一步迁移业务路由到统一应用异常类型 |
| M6-4 | 字段注册表重构 | 中 | 未开始 | - | 后端 | 由硬编码元数据迁移为 FieldHandler/Registry 可扩展架构 |
| M6-5 | 前端 Service 层 | 中 | 未开始 | - | 前端 | 抽离高频业务请求与组件状态，统一请求和缓存边界 |
| M6-6 | 类型补全与全量回归 | 高 | 进行中 | - | 全栈 | 当前已通过 TypeScript、ESLint、Python compileall 和 25 项后端测试，待 M6 收尾后完成全量验收 |
