...
最近更新：2026-09-26（完成部门协同共享与文件同步架构基线，定义组织角色、责任品类、字段权限、`.pwshare` 包协议、冲突合并和未来中继服务器演进）
...
## 三点九、P1-10 - 项目方案版本与风险跟踪基础闭环

| ID | 任务 | 优先级 | 状态 | 实际完成 | 负责模块 | 备注 |
|----|------|-------|------|---------|---------|------|
| P1-10 | 轻量项目方案版本与风险跟踪基础闭环 | 高 | 已完成 | 2026-08-19 | 全栈 | 新增方案版本/变化点/地区、RiskCase、风险专利/方案关联、追加式风险评估和持续复核；详情页风险分区接入方案/风险录入；旧风险投影改为只读，正式结论校验方案版本/法域/分析人/确认人并生成 input_hash；53 项后端测试、ESLint、TypeScript、Vite build 通过 |
...
| G0-9 | 导入、数据库 AI 与专利-项目关系可靠性闭环 | 高 | 已完成 | 2026-08-20 | 全栈 | 空映射保留原始列、缺标题/缺身份行不再整行丢弃；AI 测试与实际调用统一配置，失败任务先落库并保留准备/执行/解析错误；详情页可独立维护项目关系并保留关系多维属性；定向后端 26 项、compileall、TypeScript、ESLint 通过 |
...
| 日期 | 任务ID | 变更内容 |
|------|--------|---------|
| 2026-09-26 | COLLAB-DESIGN-1 | 完成部门协同共享与文件同步架构设计：盘点现有本地 SQLite、多数据库、库成员、组织字段、外部 Provider 同步和附件/历史能力；定义部门领导、系统管理员、组长、成员和只读协作者角色；新增责任品类多对多关系、同品类跨组协作、字段级权限、同步包/检查点/冲突/tombstone 模型；设计加密签名 `.pwshare` 快照与增量协议、收发箱和审计；规划第一阶段离线文件同步以及未来局域网/中继服务器演进。详见 `docs/21-部门协同共享与文件同步架构设计文档.md`。 |
...
| 2026-09-21 | 语义检索大阶段收口 | 完成 SQLite FTS5 `trigram`/BM25 稀疏投影，Patent trigger 更新和 ILIKE 回退；新增评测数据集、Case、Run、Result 及 Recall@K/Precision@K/MRR/NDCG@K/零结果率；评测可指定 staging index，质量门禁按同 Profile/同 index version 的 `passed` 运行阻止未验证索引激活；语义检索管理台接入 Profile 创建/受限编辑、Provider 编辑/测试/启停、索引任务、Case 增删停用和评测运行；新增任务详情/人工重试、租约过期回收、retry_wait 与 dead-letter。通过语义专项 18 项、后端全量 148 项、Python compileall、ESLint、TypeScript、Vite build 和 git diff --check。真实规模基准、真实中文评测集及 Windows 安装包升级/回滚仍属于发布前门禁。 |
...
| 2026-08-20 | G0-9 | 完成导入、数据库 AI 与专利-项目关系可靠性闭环：空映射固定为来源保留，显式 `__skip__` 才是跳过；有官方号码无标题创建 `created_pending_title`，无身份有内容进入 `retained_source_row`；LLM 配置、URL、超时、重试和错误转换统一于 `llm_service`，快速抽取和标准 AI 字段任务均先落库，网络/响应解析/字段准备失败都可在任务列表查询；AI 非 JSON 响应不再写入空结果；详情页项目关系支持直接添加/移除，后端以 `PatentProjectLink` 校验并保留关系属性，非法项目不会破坏旧关系。定向后端 26 项通过，Python compileall、ESLint、TypeScript 通过。 |
