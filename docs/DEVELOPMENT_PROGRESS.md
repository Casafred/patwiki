...
最近更新：2026-08-17（完成 G0-7 统一专利身份、高频业务视图和工作文件模板基础闭环；G0-8 治理恢复保持已完成）
...
| G0-7 | 专利统一身份与高频业务视图模板 | 高 | 已完成 | 2026-08-17 | 全栈 | 新增 `PatentIdentifier` 身份索引与格式别名；导入联合申请/公开/授权号并隔离跨记录冲突；六类高频 SavedView 幂等初始化；新增 `PatentExportTemplate`、Excel/Word/CSV 模板 API 与字段血缘输出；前端补齐身份/模板类型与 API 调用；46 项后端测试通过 |
