...
- 行没有标题但有申请号、公开号或授权号：创建标题为“待补全”的身份锚定专利，行状态为已解析/标题待补全；该标题不是外部事实，后续标题导入必须可增量补齐；
- 行没有任何官方身份标识但存在非空内容：不伪造 Patent，保存 `ImportSourceRow` 和 `FieldObservation` 为来源待补身份；完全空行才允许计为跳过；
...
- Excel 映射的空目标语义固定为“保留原始列，待治理”；只有显式目标 `__skip__` 才代表用户确认跳过治理队列。前端不得把空目标解释为丢弃，后端不得因为未知列阻断其他合法列。
- AI 设置测试和实际 AI 任务必须使用同一个配置解析、Base URL 规范化、请求协议、超时和错误转换边界。连接测试成功不代表任务可以读取另一套配置；任务错误必须带阶段（准备、网络、响应解析或写入）并持久化到 `AITask`。
- 快速 AI 抽取必须先创建 `AITask` 再创建字段或启动后台调用。字段准备、配置缺失、网络失败、响应格式错误和后台未捕获异常都必须出现在任务列表中；AI 只能写草稿字段，不能绕过风险/外部事实/关系字段治理。
- 专利详情的项目关系必须支持独立的 `GET /patents/{id}/projects` 和 `PUT /patents/{id}/projects` 维护。项目不存在时服务层拒绝并保留当前关系；添加/移除不应要求用户先提交整篇专利编辑表单。
...
项目方案版本和风险跟踪基础闭环已经进入实现：具体字段、路由、服务层门禁、详情页交互和测试要求以 `27-agent-project-risk-module-contract.md` 为准。当前实现仍严格保持轻量：没有 Artifact 关系、完整审批、Docket、法务预算或 Gate 自动阻断。

实现时必须注意：风险案例可以存在草稿评估，但草稿不能更新 `RiskCase.current_*`；正式评估必须明确方案版本和法域，并由服务端生成 `input_hash`。旧 Patent 风险字段只允许兼容读取和后续聚合投影，任何直接写入都应被拒绝或保留为来源观察。
...
文件导入和 Excel 粘贴必须使用 `preview -> confirm -> changes -> review -> apply -> rollback` 的阶段边界。`POST /import/confirm` 只写入原始附件、`ImportBatch`、`ImportSourceRow` 和 `FieldObservation`，状态为 `review_required`，不得在该请求中新增或修改 `Patent`、结构化关系或正式字段值。只有 `review` 固化逐观察项决定后，`apply` 才能写入；`rollback` 必须检查后续修改并在冲突时拒绝覆盖。

固定行语义：有公开号但没有标题是 `created_pending_title`；缺少公开号但存在任何其他内容是 `retained_source_row`；完全空行是 `skipped_empty_row`；身份冲突、解析失败和完整性错误才是 `quarantined`。Agent 不得以缺少标题、申请人、日期或其他非核心字段为由丢弃合法行，也不得以申请号/授权号猜测缺失的公开号。

导入结果中的预审计数只能标识候选变化，不能冒充已写入数量；执行结果必须区分创建、更新、复核无变化、保留来源、空行跳过、隔离和错误，并给出可追溯的批次 ID。
...
- 侧栏必须能直接看到并打开六类固定 `template_key` 的高频视图；下拉框可以保留作为完整视图列表，但不能成为唯一入口。
- 专利工作区必须提供“工作文件”入口，先读取 `/export/templates?database_id=...`，再让用户选择模板并看到格式、版本、关联视图和字段数量。
- 模板下载必须调用 `/export/excel`、`/export/word` 或 `/export/csv` 的 `template_id`；前端不得复制系统模板的字段列表或筛选语义。当前搜索和临时筛选只能作为附加条件传入，不能覆盖模板定义的身份、字段和主视图关系。
- 下载文件名必须包含模板名称和版本；下载失败要留在弹窗内显示可重试错误，不能静默关闭或清空当前工作区。
- G0-7b 的验收至少包括：六类视图按钮可切换；模板列表按当前数据库加载；Excel/Word/CSV 三种格式能按模板下载；模板版本随下载请求生效；空模板列表和失败状态可理解；`npm run lint`、TypeScript 和 Vite build 通过。

#### E. 视图列配置契约（G0-7c）

本节是执行视图字段自定义的 Agent 合同。涉及 `PatentListPage`、字段注册表、视图 API 或导出模板的 Agent 必须按以下规则实现并补测试：

- `PatentView.column_config` 是当前视图的字段投影，配置项固定为 `{key, visible, width, order}`；`width` 范围为 40-1200，`order` 为非负整数，同一视图不得有重复 key。
- 六类高频视图只保存一份配置，不复制 `Patent`、不创建同名业务事实表。显隐、顺序和宽度只改变当前视图；单元格编辑仍写入统一专利主数据并遵守来源/Wiki 规则。
- 视图存在非空 `column_config` 时，以配置为准：`visible=true` 才显示；配置中缺少的已知字段和新导入字段默认隐藏，但必须在列管理中可选。未注册 key 不得因保存配置而丢失，便于字段注册表演进。
- 普通无视图列表可以继续使用浏览器本地显隐/列宽兼容设置，但任何高频保存视图不得只写 `localStorage`。前端必须通过 `PUT /views/{view_id}` 持久化，并在保存成功后刷新本地视图状态。
- “列管理”至少提供字段搜索、显示/隐藏、上移/下移、列宽编辑、全部显示/隐藏、恢复初始配置、取消和明确保存状态。拖动表头改变列宽后也必须最终写回当前视图。
- 新字段进入字段注册表、待治理属性或人工自定义字段流程时，不得自动污染已有高频视图的显示列；用户必须能在指定视图中主动加入。
- 工作文件导出使用 `PatentExportTemplate.field_keys`，不能因为用户改变当前视图列就擅自改变模板字段；只有后续明确实现“从当前视图创建模板”时才允许建立该关系。

视图列切片的最小验收：更新后重新读取配置仍保留显隐/顺序/宽度；两个视图配置互不影响；未知 key 可保留；重复 key 被拒绝；`Patent` 内容不因列配置变化；前端 ESLint、TypeScript、Vite build 和后端全量测试通过。
