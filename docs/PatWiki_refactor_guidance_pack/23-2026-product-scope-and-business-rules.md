...
- 系统暂时未知的属性不视为导入失败：原始列、单元格和来源行必须保留，可查询、可导出、可提交字段候选，确认语义后再回填为正式字段；真正的文件损坏、身份冲突或解析错误才进入隔离。
...
4. **高效交互表格与人工录入**：快速收录、详情页就地编辑、Excel 粘贴/导入、搜索、排序、分组、列配置、批量编辑、关系钻取、草稿恢复和保存视图；具体要求见 `26-human-data-entry-interaction-spec.md`。
