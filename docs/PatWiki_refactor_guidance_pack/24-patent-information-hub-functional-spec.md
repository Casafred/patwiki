...
> 更新：2026-08-15
> 本文把 `23-2026-product-scope-and-business-rules.md` 的产品范围继续细化为详情页、专利身份、增量导入、Wiki 历史和高频表格视图规格。发生冲突时，业务范围仍以 23 号文档为准。
...
flowchart TD
    ...
    ROOT -->|否且缺主公开号| QUARANTINE["隔离：补充身份后再导入"]
```
...
来源观察不能因为与当前值相同就被丢弃，否则无法回答“这张表在这一天提供了什么信息”。
...
每个非空导入字段形成 `FieldObservation`：canonical field、raw value、normalized value、旧采用值、候选值、差异类型、默认动作、最终决定、决定人和决定时间。
...
2. ImportBatch 来源元数据、行级匹配预览和字段差异分类。
3. `PatentImportEvent + FieldObservation`，接入 Wiki 历史聚合读取。
4. 专利详情聚合 API 和十页信息架构，先迁移现有字段，再逐页补新关系。
5. 六个已确认高频 SavedView 的数据集与默认列，先支持 Excel 导出。
6. 收集真实导出样式后增加 ExportTemplate 版本和 Word ReportTemplate。
...
7. 任取导入后的一个字段，可追溯到工作簿、表格标题、工作表、行号、原始值、规范值和用户决定。
8. 六个高频视图都能从同一专利事实和关系生成 Excel，修改一次专利著录后无需到六张台账重复修改。
