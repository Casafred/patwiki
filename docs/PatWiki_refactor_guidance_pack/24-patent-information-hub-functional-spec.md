...
| 5. 同族、引用与法律状态 | 同族成员公开号、引用/被引用公开号、国家/地区、关系方向、申请/公开/授权信息、当前法律状态、法律状态事件和来源时间 | 先显示可点击同族与引用清单；点击已入库成员进入其详情页；关系图用于补充探索；支持同族聚拢列表和比较法域 | `PatentFamily + Citation + PatentIdentifier + FamilyRelation + LegalStatusEvent`；原始关系列另保留来源投影 |
...
- 同族成员必须保留为独立 `Patent` 记录；详情页的“同族专利”区域必须返回成员 `patent_id`、公开号、申请/授权号、国家/地区、法律状态和关键日期。
- 成员清单中的每一行都是可操作导航。点击后打开被点击成员的详情页，而不是只改变图谱选中态；关系图节点也必须提供同样的“打开专利详情”动作。
- 同族清单和关系图查询必须限制在当前专利所属 `database_id`，不能因为历史数据复用了 `family_id` 而跨库显示成员。
- 列表“同族聚拢”是视图展示投影：按当前数据库/视图的筛选结果按族生成组头，组内仍渲染每一篇独立专利。专利没有 `family_id` 时作为单行展示。
- 聚拢组头支持展开/收起，分页、筛选、排序、勾选、导出和详情跳转仍作用于原始专利行；聚拢不能复制、合并或改变 Patent 数据。
- 保存视图的字段分组和同族聚拢是两个展示维度；当前交互开启同族聚拢时，以同族组头为主，避免嵌套重复组头。
...
### 4.5 专利关系类型、原始列与结构化关系

关系是专利信息中心的正式数据类型。当前第一批关系键固定为：

| canonical relation key | 来源列显示名 | 关系方向 | 结构化实体 | 读取状态 |
|---|---|---|---|---|
| `family_members` | 同族专利号/同族公开号 | 当前专利 ↔ 同族成员 | `PatentFamily`（当前实现兼容 `Patent.family_id`） | `in_database` / `other_database` / `missing_record` |
| `cited_patents` | 引用专利号/引用专利 | 当前专利 → 被引用专利 | `Citation(citing_patent_id=current, cited_patent_id=target)` | `in_database` / `other_database` / `missing_record` |
| `citing_patents` | 被引用专利号/被引用专利 | 引用专利 → 当前专利 | `Citation(citing_patent_id=source, cited_patent_id=current)` | `in_database` / `other_database` / `missing_record` |

Agent 必须同时维护两条数据路径：

1. **原始来源投影**：导入单元格的完整文本保留在来源行、字段观察和当前专利的关系字段投影中。分隔符、顺序、重复写法和原始空白不能因为解析而丢失；列表页、详情页和导出可以继续显示这列。
2. **结构化关系**：把单元格解析出的公开号按统一身份规则匹配当前数据库中的 `Patent`，再建立 `PatentFamily` 或 `Citation`。关系实体只负责关系、方向、导航和状态，不取代原始列。

导入同族/引用列时，必须先保存来源观察再解析关系。目标号码已存在时建立或复用关系；目标号码不存在时保留原始值并返回 `missing_record`，默认不得为一次普通关系导入创建隐形占位专利。关系读取接口不得创建记录或修改数据库。若目标在其他数据库，应显示 `other_database`，可以展示其号码和摘要信息，但当前数据库的图谱、聚拢和默认列表不得把它当作当前库成员。

详情页必须提供确定性的同族和引用清单：

- 当前库目标：显示“当前数据库”和“查看详情”，使用 `patent_id` 导航；
- 其他库目标：显示“其他数据库”，不把它伪装成当前库可编辑记录；
- 未入库目标：显示“未入库”，保留公开号，不生成错误详情链接；
- 引用清单至少分为“我引用的专利”和“引用我的专利”，不能只靠无方向关系图表达；
- 关系图只能作为探索补充，必须遵守相同的数据库隔离规则。
...
- 同族/引用多值列：保留原始文本作为来源投影；规范化号码用于匹配和关系实体，关系新增/重复/缺失均形成可追溯观察，不因单个目标缺失而丢弃整行。
...
12. 同族成员清单中的成员可直接打开各自详情；同族图谱节点不允许停留在不可导航的选中状态。
13. 开启同族聚拢后，同族组头可展开/收起，组内记录仍按专利 ID 独立存在；无族专利不被隐藏或错误归入其他组。
14. 导入“引用专利号”和“被引用专利号”各含多个公开号时，原始两列仍可在专利列表、详情和导出中读取，且结构化关系分别落入正向/反向引用。
15. 引用目标已在当前库、仅在其他库、尚未入库时，详情清单分别显示 `in_database`、`other_database`、`missing_record`；未入库目标不创建占位专利，读取接口无数据库副作用。
16. 已有引用关系再次导入必须幂等，不增加重复 `Citation`，但仍新增本次来源观察、导入时间和来源表标题。
17. 关系原始投影不能通过普通单元格、详情编辑或批量更新接口改写；关系导入失败时保留原始行并返回可重试错误。
