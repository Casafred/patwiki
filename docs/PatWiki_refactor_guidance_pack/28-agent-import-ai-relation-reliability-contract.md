...
## 7. G0-12 主表批量与历史可读性契约

### 7.1 批量命令

Agent 实现主表多选功能时必须复用以下服务/API边界：

| 命令 | 用途 | 关键约束 |
| --- | --- | --- |
| `POST /patents/bulk-update` | 统一字段批量赋值 | 先校验全部 ID；空值不修改；风险正式投影和关系原始列拒绝写入 |
| `POST /patents/bulk-tag` | 标签追加/替换/移除 | `mode` 必须是 `add`/`replace`/`remove`；无标签选择不能提交 |
| `POST /patents/bulk-move-database` | 移库 | 目标库有效且未归档；跨库视图自动清空；写 `PatentHistory` |
| `POST /patents/bulk-move-view` | 移动到同库视图 | 目标视图必须属于每一条选中专利所在库；空目标表示库主表 |
| `POST /patents/bulk-duplicate` | 创建工作副本 | 清空官方身份与正式风险投影；写 `duplicate_of` 与来源说明；保留研究内容 |

批量命令的服务层必须先加载全部选中记录。发现缺失 ID、目标不存在、跨库目标或枚举值错误时，必须在提交前抛出可读异常，不得只成功一部分。命令完成后返回实际数量，不能以前端传入数量冒充成功数量。

### 7.2 前端批量交互

- 选择栏至少提供批量编辑、批量标签、移动视图、移库、复制工作副本和 AI 字段批量处理。
- AI 字段批量处理弹窗必须把“处理范围”和“目标字段”分成两个明确步骤；列菜单、填充柄只负责预选范围/字段并打开弹窗，不能绕过任务确认直接执行。
- 批量编辑应保留高频字段快捷输入，同时提供已注册可编辑字段选择器；关系字段、公式字段、Link/Lookup/Rollup 和风险正式投影不可出现在通用选择器。
- 任何批量失败都要保留选择状态和用户输入，页面不能只显示 `NetworkError` 后关闭弹窗。

### 7.3 历史聚合

- `PatentHistory` 仍以字段级记录为审计事实，不得为折叠而新增汇总替代记录或删除明细。
- 详情历史 UI 对 `source=import` 优先按 `import_batch_id` 聚合，默认只显示来源表、时间、来源行和字段变更数；用户展开批次后查看旧值/新值明细。
- 没有批次 ID 的旧导入记录只能按来源表和时间窗口做保守分组，不能把不同导入静默合并。

### 7.4 风险视图关联投影

- `ip_risk_control` 必须包含已注册的 `product_id` 和 `projects` 列；已有用户自定义列配置只能追加缺失列，不能覆盖用户排序、显隐和宽度。
- `product_id` 是产品外键的显示投影，前端显示产品名称；`projects` 是 `PatentProjectLink` 的只读名称投影。两者不得改写为逗号文本，也不允许用视图本地数组替代关系实体。
