...
>
> 文档状态：`implementation-guided / v1.3 / 2026-08-15`。本包同时包含 2026 范围基线和长期目标架构；除明确标注“当前已具备”的能力外，均不得视为当前产品已上线功能。
...
1. 先读 `23`，确认今年只优先建设专利信息中心及已确认业务规则。
2. 再读 `24`，确认详情页、号码身份、导入和首批高频视图的具体行为。
3. 阅读 `21`，确认当前单机 SQLite 原型的实施边界及迁移门禁。
4. 阅读 `15`、`16`，冻结专利身份、关系和字段语义。
5. 按需要阅读 `17`、`18`、`19`，确认风险、责任数据和自动化的长期边界。
6. 阅读 `22` 理解完整产品形态，但不得用长期工作流改写当前 P0。
7. 以 `20` 的 0A-0E 阶段验收拆分首批实现 PR。
