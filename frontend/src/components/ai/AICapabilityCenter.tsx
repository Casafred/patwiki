...
type CenterTab = 'models' | 'fields' | 'semantic' | 'tasks'
...
export default function AICapabilityCenter() {
  ...
  return <div className="workspace-page ai-capability-center"><div className="workspace-page-shell">
    ...
    <nav className="ai-center-tabs" aria-label="AI 能力模块"><button className={tab === 'models' ? 'active' : ''} onClick={() => setTab('models')}>模型与供应商</button><button className={tab === 'fields' ? 'active' : ''} onClick={() => setTab('fields')}>AI 字段管理</button><button className={tab === 'semantic' ? 'active' : ''} onClick={() => setTab('semantic')}>语义检索工作台</button><button className={tab === 'tasks' ? 'active' : ''} onClick={() => setTab('tasks')}>任务与运行记录</button></nav>
    <div className="ai-center-content">{tab === 'models' && <SettingsPage />}{tab === 'fields' && <FieldSettingsPage />}{tab === 'semantic' && <SemanticSearchManagement />}{tab === 'tasks' && <AITaskMonitor />}</div>
  </div></div>
}
