...
const emptyDepartment: DepartmentForm = { name: '', description: '' }
const emptyPerson: PersonForm = { name: '', email: '', department_id: '', role: '', notes: '', is_active: true }
const emptyProductLine: ProductLineForm = { name: '', code: '', description: '' }
...
export default function ManagementPage() {
  ...
  const saveDepartment = async () => {
    ...
    setSaving(true); setError('')
    try {
      const payload = { name: departmentForm.name.trim(), description: departmentForm.description.trim() || undefined }
  ...
  const saveProductLine = async () => {
    ...
    setSaving(true); setError('')
    try {
      const payload = { name: productLineForm.name.trim(), code: productLineForm.code.trim() || undefined, description: productLineForm.description.trim() || undefined }
  ...
  const renderOrganization = () => (
    <>
      ...
      <div className="management-split">
        <section><div className="management-section-title"><strong>部门</strong><button className="btn btn-secondary" onClick={() => { beginCreate(); setShowDepartmentForm(true) }}>新增部门</button></div>{(showDepartmentForm || editingDepartmentId !== null) && <div className="management-form compact"><FormField label="名称"><input className="form-input" style={inputStyle} value={departmentForm.name} onChange={e => setDepartmentForm({ ...departmentForm, name: e.target.value })} /></FormField><div style={{ marginTop: 12 }}><FormField label="描述"><textarea className="form-input" style={{ ...inputStyle, minHeight: 60 }} value={departmentForm.description} onChange={e => setDepartmentForm({ ...departmentForm, description: e.target.value })} /></FormField></div><div className="management-form-actions"><button className="btn btn-primary" disabled={saving} onClick={() => void saveDepartment()}>{editingDepartmentId ? '保存修改' : '创建部门'}</button><button className="btn btn-secondary" onClick={() => { setEditingDepartmentId(null); setDepartmentForm(emptyDepartment); setShowDepartmentForm(false) }}>取消</button></div></div>}<TableShell><TableHead><Th>部门</Th><Th>人员</Th><Th>操作</Th></TableHead><tbody>{departments.map(department => <tr key={department.id}><Td><strong>{department.name}</strong></Td><Td>{people.filter(person => person.department_id === department.id).length}</Td><Td><RowActions onEdit={() => { setEditingDepartmentId(department.id); setDepartmentForm({ name: department.name, description: department.description || '' }) }} onDelete={() => void remove(`部门“${department.name}”`, () => departmentApi.delete(department.id), () => { setDepartments(current => current.filter(item => item.id !== department.id)); setPeople(current => current.map(person => person.department_id === department.id ? { ...person, department_id: undefined } : person)) })} /></Td></tr>)}</tbody></TableShell>{departments.length === 0 && <EmptyState text="暂无部门" />}</section>
