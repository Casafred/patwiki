...
export default function ViewColumnConfigPanel({ open, view, fields, onClose, onSave, onRemove }: ViewColumnConfigPanelProps) {
  ...
  useEffect(() => {
    // Rehydrate the draft when asynchronously loaded view/field metadata changes.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    if (!saving) setDraft(initialConfig)
  }, [initialConfig, saving])
  if (!open) return null
  ...
  const renderColumn = (column: ViewColumnConfig, targetVisible: boolean) => {
    ...
    return <div key={column.key} draggable onDragStart={event => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', column.key); setDraggedKey(column.key) }} onDragEnd={() => setDraggedKey(null)} onDragOver={event => event.preventDefault()} onDrop={event => { event.stopPropagation(); const sourceKey = draggedKey || event.dataTransfer.getData('text/plain'); if (sourceKey) moveColumn(sourceKey, column.key, targetVisible); setDraggedKey(null) }} style={{ display: 'grid', gridTemplateColumns: '26px minmax(180px, 1fr) 82px 74px', gap: 8, alignItems: 'center', padding: '8px 10px', borderBottom: '1px solid #f1f5f9', background: targetVisible ? '#fff' : '#f8fafc', opacity: targetVisible ? 1 : 0.72, cursor: 'grab' }}>
      ...
      <div className="modal" style={{ maxWidth: 760 }} onClick={event => event.stopPropagation()}>
        ...
        <div className="modal-body">
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 10 }}>
            <input className="form-input" value={search} onChange={event => setSearch(event.target.value)} placeholder="搜索字段" style={{ flex: '1 1 220px', minWidth: 180 }} />
          ...
          <div style={{ fontSize: 12, color: '#64748b', marginBottom: 8 }}>共 {draft.length} 个字段，当前显示 {visibleCount} 个。拖动字段条即可调整顺序；拖到另一栏会同步显示状态。</div>
          <div style={{ maxHeight: 500, overflowY: 'auto', display: 'grid', gap: 10 }}>
            <section
              style={{ border: '1px solid #c7e3dd', borderRadius: 6, overflow: 'hidden' }}
              onDragOver={event => event.preventDefault()}
              onDrop={event => {
                const sourceKey = draggedKey || event.dataTransfer.getData('text/plain')
                if (sourceKey) moveColumnToEnd(sourceKey, true)
                setDraggedKey(null)
              }}
            >
              <div style={{ padding: '8px 10px', background: '#eefaf7', color: '#226d65', fontSize: 12, fontWeight: 700 }}>当前显示字段 ({visibleCount})</div>
              {visibleColumns.map(column => renderColumn(column, true))}
              {visibleColumns.length === 0 && <div style={{ padding: 20, color: '#94a3b8', fontSize: 12, textAlign: 'center' }}>拖到此处显示字段</div>}
            </section>
            <section
              ...
              onDrop={event => {
                const sourceKey = draggedKey || event.dataTransfer.getData('text/plain')
                if (sourceKey) moveColumnToEnd(sourceKey, false)
                setDraggedKey(null)
              }}
            >
              <div style={{ padding: '8px 10px', background: '#f8fafc', color: '#64748b', fontSize: 12, fontWeight: 700 }}>当前隐藏字段 ({draft.length - visibleCount})</div>
              {hiddenColumns.map(column => renderColumn(column, false))}
              {hiddenColumns.length === 0 && <div style={{ padding: 20, color: '#94a3b8', fontSize: 12, textAlign: 'center' }}>拖到此处隐藏字段</div>}
            </section>
          </div>
